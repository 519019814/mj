<template>
  <div class="feedback-container">
    ...
    >
      ...
      <el-table-column label="所属公厕" min-width="110px">
        <template slot-scope="{ row }">
          <span>{{ row.title }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="是否处理" sortable="custom" align="center">
        <template slot-scope="{ row }">
          <el-tag :type="row.wc % 2 === 0 ? 'success' : 'danger'">{{ row.wc % 2 === 0 ? '已处理' : '未处理' }}</el-tag>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="{ row, $index }">
          <el-button type="primary" size="mini" icon="el-icon-edit" @click="handleFeedback(row.id)">
            处理
          </el-button>
